...
bool Spell::doYouKnowIt(int a) {
  if (a < 0 || a > 50) return false;
  if (knowledge[a]) return true;
  else return false;
}

int Spell::get(int a, int b) {
  if (a < 0 || a > 8 || b > 3 || b < 0) {
...
void Spell::set(int a, int b, int value) {
  if (a < 0 || a > 8 || b > 3 || b < 0) {
