...
Character::Character(Random_Generator_ *Rand_gen, int storyline_, int exp, int levl, int Stre, int Dext,
  ...
  Starting_Health();
  if (class_type.get(0) == 0) armor_class = kBarbarian_Unarmored_Defence + DexModifier + ConModifier;
  level = levl;
  Starting_Maxhealth();
...
void Character::Set(int what, int value) {// what - what parameter will be changed, value - modifier(can be negative)
  switch (what) {
    ...
    case 18: {
      ...
      class_type.Create(allowance, value, skills_b);
      health_dice = class_type.get(kGetHealth_dice);
      for (int i = 0; i < kSkills_Num; i++) {
        if (class_type.get(i + kClassType_get_shift) == 1) {
          skills_b[i] = true;
        }
      }
      delete allowance;
...
int Character::Get(int what) {
  ...
  else if (what == 23) { return class_type.get(0); }
  else if (what == 24) { return 0; }
  else if (what == 25) { return class_type.get(39); }//archetype
...
bool Character::Get_bool(int what) {
  if (what == 0) { return advantage; }
    ...
    Existing_Types E_T;
    //cout << "\nCharacter: what = "<< what << E_T.params_b[what] << " = " << class_type.get_bool(what - kClassType_get_bool_shift) << endl;
    return class_type.get_bool(what - kClassType_get_bool_shift);
  }//kClassType_get_bool_shift
  else if (what > 34 && what < 51) { return state[what - kStates_shift]; }
    //temproary
  else if (what > 50 && what < 57) { return class_type.get_bool(what - 51); }
  return false;
}
  ...
  if (choose_to_proceed == 2) {
    ...
    while (some_more) {
      ...
      int limit[kItem_For_Sale_Types + 1] =
          {0, kWeapon_NUM, kRanged_Weapon_NUM + limit[1], kAmmo_NUM + limit[2],
           kArmor_NUM + limit[3], kGoods_NUM + limit[4],
           kSpellCastingFocus_NUM + limit[5], kShield_NUM + limit[6],
           kRing_NUM + limit[7], kPotion_NUM + limit[8],
      ...
      for (int i = limit[item_ - 1]; i < limit[item_]; i++) {
        ...
        else if (item_ == 13) { cout << E_I.prices_of_$_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 14) { cout << E_I.prices_of_INS_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 15) { cout << E_I.prices_of_TAH_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 16) { cout << E_I.prices_of_EXP_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 17) { cout << E_I.prices_of_MNT_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 18) { cout << E_I.prices_of_VEH_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 19) { cout << E_I.prices_of_TG_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 20) { cout << E_I.prices_of_GS_merchants_items[i - limit[item_ - 1]]; }
        else if (item_ == 21) { cout << E_I.prices_of_T_merchants_items[i - limit[item_ - 1]]; }
        //else if (item_ > 9 && item_ < 24) { cout << E_I.prices_of_merchants_items[i - limit[item_ - 1]]; }
        cout << endl;
      }
      int itemChosen = IsNumber<int>(item_, limit[item_ - 1], limit[item_]);
      if (itemChosen - 1 < kItems_Without_JSON_NUM) name_ = E_I.All_s[itemChosen - 1];
      else name_ = E_I.names_of_merchants_items[itemChosen - kItems_Without_JSON_NUM - 1];//9-0790-90790-790-
      int quantity = 1;
      ...
      inputJson.open("E:/Den`s/programming/Git_c++/Another_DnD_simulator/aditional_tools/5etools json/items/items.json");
...
void Character::SetClass(Allowance *allowance, Random_Generator_ *Rand_gen) {
  ...
  class_type.Create(allowance, class_type_ - 1, skills_b);
  health_dice = class_type.get(kGetHealth_dice);
  proficiency = ProficiencySetter();
...
bool Character::Load(int parameter_i[], bool parameter_b[], vector<string> persona, vector<int> item_) {
  ...
  class_type.Load(parameter_i[kClass_type_parameter], parameter_b, parameter_i[kClass_archetype_parameter]);
  proficiency = ProficiencySetter();
