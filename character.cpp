...
class character {
 ...
 public:
  ...
  void setF(string a, string b, string story, int l, int h, int s, int d, int c, int i, int w, int cha) {
    ...
    if (story == "Acolyte") {
      if (skill.s[6] == WisM)skill.s[6] += prof;
      if (skill.s[14] == IntM)skill.s[14] += prof;
    } else if (story == "Charlatan") {
      if (skill.s[4] == ChaM)skill.s[4] += prof;
      if (skill.s[15] == DexM)skill.s[15] += prof;
    } else if (story == "Criminal") {
      if (skill.s[4] == ChaM)skill.s[4] += prof;
      if (skill.s[16] == DexM)skill.s[16] += prof;
    } else if (story == "Entertainer") {
      if (skill.s[0] == IntM)skill.s[0] += prof;
      if (skill.s[12] == ChaM)skill.s[12] += prof;
    } else if (story == "FolkHero") {
      if (skill.s[1] == WisM)skill.s[1] += prof;
      if (skill.s[17] == WisM)skill.s[17] += prof;
    } else if (story == "GuildArtisan") {
      if (skill.s[6] == WisM)skill.s[6] += prof;
      if (skill.s[13] == ChaM)skill.s[13] += prof;
    } else if (story == "Hermit") {
      if (skill.s[9] == WisM)skill.s[9] += prof;
      if (skill.s[14] == IntM)skill.s[14] += prof;
    } else if (story == "Noble") {
      if (skill.s[5] == IntM)skill.s[5] += prof;
      if (skill.s[13] == ChaM)skill.s[13] += prof;
    } else if (story == "Outlander") {
      if (skill.s[3] == StrM)skill.s[3] += prof;
      if (skill.s[17] == WisM)skill.s[17] += prof;
    } else if (story == "Sage") {
      if (skill.s[2] == IntM)skill.s[2] += prof;
      if (skill.s[5] == IntM)skill.s[5] += prof;
    } else if (story == "Sailor") {
      if (skill.s[3] == StrM)skill.s[3] += prof;
      if (skill.s[11] == WisM)skill.s[11] += prof;
    } else if (story == "Soldier") {
      if (skill.s[3] == StrM)skill.s[3] += prof;
      if (skill.s[7] == ChaM)skill.s[7] += prof;
  ...
  int get(int a) {
    ...
    if (a == 1) { return experience; }
      ...
      string message = "Failures: " + to_string(deathSavesF) + " Successes: " + to_string(deathSavesS);
    } else if (a == 11) {
      printf("%s \n", "reserved for skills");
      return 0;
    } else if (a == 12) {
  ...
  void levelUp() {
    if (level < 4 && experience > 300 * pow(3, level - 1)) {
      ...
      if (lv == 1) {
        int tHP = rand() % diceOfHealth - 1 + 1;
        printf("%s %d \n", "Rolled Health + Constitution modifier:", tHP + ConM);
        maxHealth += tHP + ConM;
        Health = maxHealth;
      }//role dices
      else {
        maxHealth += diceOfHealth / 2 + 1;
        Health = maxHealth;
      }//take middle
      printf("%s %d %s \n",
             "You reached",
      ...
      levelUp();
      /*printf("%s \n", "reserved for skill levelUp");*/} else if (level == 4 && experience > 6499) {
      level++;
      printf("%s \n",
             "Character leveled up, your health increased, choose the way: roll dices(1) or take a middle(2)?");
      int lv = 0;
      ...
      levelUp();
      /*printf("%s \n", "reserved for skill levelUp");*/} else if (level == 6 && experience > 22999) {
      level++;
      printf("%s \n",
             "Character leveled up, your health increased, choose the way: roll dices(1) or take a middle(2)?");
      int lv = 0;
      ...
      levelUp();
      /*printf("%s \n", "reserved for skill levelUp");*/} else if (level == 7 && experience > 33999) {
      level++;
      prof++;
      printf("%s \n",
             "Character leveled up, your health increased, choose the way: roll dices(1) or take a middle(2)?");
      ...
      levelUp();
      /*printf("%s \n", "reserved for skill levelUp");*/} else if (level == 8 && experience > 47999) {
      level++;
      printf("%s \n",
             "Character leveled up, your health increased, choose the way: roll dices(1) or take a middle(2)?");
      int lv = 0;
  ...
  void setSkill(skills c) {
    for (int j = 0; j < 18; j++) {
      if (j == 0 || j == 2 || j == 5 || j == 8 || j == 10 ||
          j == 14) { c.s[j] += IntM; }//acrobatics,arcana,history,investigation,nature,religion
      else if (j == 1 || j == 6 || j == 9 || j == 11 ||
          j == 17) { c.s[j] += WisM; }//animalHandling,.s[6],medicine,perception,survival
      else if (j == 3) { c.s[j] += StrM; }//athletics
      else if (j == 4 || j == 7 || j == 12 ||
          j == 13) { c.s[j] += ChaM; }//deception,intimidation,performance,persuasion
